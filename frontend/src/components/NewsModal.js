...
const NewsModal = ({ show, handleClose, article }) => {
  ...
  const shareUrl = `https://feedfusion.vercel.app/api/news/article?title=${encodeURIComponent(article.title)}&link=${encodeURIComponent(article.link)}&description=${encodeURIComponent(article.description)}&image=${encodeURIComponent(article.image)}`;
